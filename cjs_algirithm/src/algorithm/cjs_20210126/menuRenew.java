...
//어떤 단품메뉴들을 조합해서 코스요리 메뉴로 구성하면 좋을 지 고민하던 스카피는 이전에 각 손님들이 주문할 때 가장 많이 함께 주문한 단품메뉴들을 코스요리 메뉴로 구성하기로 했습니다.
	...
	public String[] solution(String[] orders, int[] course) {
        ...
        for(int i=0;i<1;i++){
	    	for(int j=0;j<orders[i].length();j++){
	    		System.out.println(orders[i].charAt(j));
	    	}
	    }
...
//        	for(int j=0;j<orders[i].length();j++){
//        		if(map.get(orders[i].charAt(j))==null){
//        			map.put(orders[i].charAt(j), 1);
//        		}else{
//        			map.put(orders[i].charAt(j), (int)map.get(orders[i].charAt(j))+1);
//        		}
//        	}
//        }
