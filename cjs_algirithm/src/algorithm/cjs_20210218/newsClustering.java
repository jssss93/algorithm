...
public class newsClustering {
    ...
    public static void main(String args[]){
    	...
    	System.out.println("answer==>"+sol.solution(str1, str2));
    }
    public int solution(String str1, String str2) {
    	...
    	for(int i=0;i<str1.length()-1;i++){
    		if(str1.substring(i, i+2).matches ("[a-z|A-Z]*")){
    			list1.add(str1.substring(i, i+2));
    		}
    	}
    	for(int i=0;i<str2.length()-1;i++){
    		if(str2.substring(i, i+2).matches ("[a-z|A-Z]*")){
...
//    	System.out.println((int)Math.floor(((float)inter.size()/(float)sum.size())*65536));
        
        if(list1.size()==0&&inter.size()==0){
        	return 65536;
        }else{
        	return (int)Math.floor(((float)inter.size()/(float)list1.size())*65536);	
        }
//    	return 0;
    }
}
